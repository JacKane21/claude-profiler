...
use crate::config::{
    ...
    ENV_DEFAULT_SONNET_MODEL, ENV_PROXY_TARGET_URL, Profile,
};
use crate::proxy;
        ...
        match action {
            ...
            Action::EditProfile => {
                if let Some(profile) = self.current_profile() {
                    ...
                    self.proxy_url_input = Input::new(proxy_url);
...
mod tests {
    ...
    fn reset_all_restores_default_config() {
        ...
        assert_eq!(app.config.profiles.len(), 5); // Default config has 5 profiles
